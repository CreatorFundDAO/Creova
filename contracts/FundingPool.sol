...
contract FundingPool is ReentrancyGuard, Ownable, AccessControl {
    ...
    uint256 public constant KYC_WITHDRAWAL_THRESHOLD = 5 ether; // Reduced from 10 ETH to 5 ETH
    ...
    function fundProject(uint256 _campaignId) external payable nonReentrant {
        ...
        uint256 totalFee = (msg.value * 50) / 1000;
        if (totalFee == 0 && msg.value > 0) totalFee = 1;
        uint256 treasuryFee = totalFee / 2;
        uint256 ownerFee = totalFee - treasuryFee;
    ...
    function withdrawFunds(uint256 _campaignId) external nonReentrant {
        ...
        uint256 totalWithdrawalFee = (amount * 25) / 1000;
        if (totalWithdrawalFee == 0 && amount > 0) totalWithdrawalFee = 1;
        uint256 treasuryFee = totalWithdrawalFee / 2;
        uint256 ownerFee = totalWithdrawalFee - treasuryFee;
